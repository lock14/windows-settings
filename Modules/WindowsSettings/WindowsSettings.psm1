...
# Repository User bin directory
$repoBinDir = Join-Path (Split-Path -Parent (Split-Path -Parent $ModuleRoot)) "bin"
if ((Test-Path $repoBinDir) -and ($env:Path -notlike "*$repoBinDir*")) {
    $env:Path = "$env:Path;$repoBinDir"
}
...
# -------------------------------------------------------------
# 2. Modern Rust CLI & Coreutils Un-Aliasing
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# 5. Load Public Functions & Tab Completers
# -------------------------------------------------------------
$publicFiles = Get-ChildItem -Path (Join-Path $ModuleRoot "Public\*.ps1") -ErrorAction SilentlyContinue
foreach ($file in $publicFiles) {
    . $file.FullName
}

$completionFiles = Get-ChildItem -Path (Join-Path $ModuleRoot "Completions\*.ps1") -ErrorAction SilentlyContinue
