...
# -------------------------------------------------------------
$wsBinDir = if ($PSScriptRoot) { Join-Path (Split-Path -Parent $PSScriptRoot) "bin" } else { Join-Path $HOME "windows-settings\bin" }
if (-not (Test-Path $wsBinDir)) {
    $wsBinDir = Join-Path $HOME "windows-settings\bin"
}
...
# -------------------------------------------------------------
# 7. CLI Argument Tab Completions
# -------------------------------------------------------------
$completionsScript = if ($PSScriptRoot) {
    Join-Path (Split-Path -Parent $PSScriptRoot) "completions\completions-setup.ps1"
} else {
    Join-Path $HOME "windows-settings\completions\completions-setup.ps1"
}
